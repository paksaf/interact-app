...
public class SwiftFlutterContactsPlugin: NSObject, FlutterPlugin, FlutterStreamHandler, CNContactViewControllerDelegate, CNContactPickerDelegate {
    // Optional: under UISceneDelegate, AppDelegate.window is nil at plugin
    // registration time — force-unwrap here EXC_BREAKPOINTs the whole app
    // (QuisApp/flutter_contacts#209 / #230). Resolve lazily when presenting.
    private var rootViewController: UIViewController?
    private var externalResult: FlutterResult?

    private static func resolveRootViewController() -> UIViewController? {
        if let fromDelegate = UIApplication.shared.delegate?.window??.rootViewController {
            return fromDelegate
        }
        ...
        for scene in scenes {
            ...
            if let any = scene.windows.first?.rootViewController {
                return any
            }
        }
        return nil
    ...
    private func presenter() -> UIViewController? {
        rootViewController ?? Self.resolveRootViewController()
    }
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "openExternalPick":
            DispatchQueue.main.async {
                ...
                self.presenter()?.present(contactPicker, animated: true, completion: nil)
                self.externalResult = result
            }
